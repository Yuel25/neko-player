...
/// Push the playlist (model, current index, count, loop label) into the UI.
/// Rebuilds the whole row model only when the player marked it dirty.
fn sync_playlist_ui(player: &MpvPlayer, ui: &MainWindow) {
...
/// Persist player state (volume, speed, loop mode, playlist, resume
/// positions) and the window placement into the config file.
fn snapshot_and_save(
    ui: &MainWindow,
    player: &MpvPlayer,
    settings_rc: &Arc<std::sync::Mutex<settings::Settings>>,
) {
    {
        let st = player.state.lock().unwrap();
