...
- 当前视频帧保存为 PNG
- 全屏、快捷键和自动隐藏控制栏(播放或暂停时闲置 1 秒整条隐藏，动鼠标即可唤回)
...
也可以运行 `neko-player-setup-1.1.2.exe` 安装。安装器支持常见音视频格式关联，会注册到 Windows「设置 → 默认应用」的候选列表，并会在资源管理器右键菜单中添加“通过 neko player 打开”。
